...
Mimir (_named after the Norse god: [Mímir](https://en.wikipedia.org/wiki/Mímir)_) is a dynamic, tree-walk interpreted language implemented in [Scala](https://www.scala-lang.org/). I understand tree-walking is not performant. This is to meet the immediate requirements of my senior project at BYU–Idaho. The first version has been switched to Scala to accelerate development speed. Following the MVP, I plan to refactor the Scala backend into a stack-based bytecode virtual machine to significantly reduce execution overhead and explore lower-level optimization techniques.
...
- **Bytecode VM**: Refactor the Scala backend from a tree-walk interpreter to a stack-based bytecode virtual machine. This will significantly reduce execution overhead and open the door to lower-level optimization techniques. The VM itself will be implemented in Odin for low-level control and performance.
